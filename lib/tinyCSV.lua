...
function csvfile:parse(infile,sep,format,looping)
    ...
    if io.open(infile,'r') then
        ...
        for i in io.lines(infile) do
            s=i;
            for key,value1,value2 in s:gmatch(string.format("%s%s%s%s%s","([%w_-]+)",sep,"([%w_.-]+)",sep,"([%w_-]+)")) do
                entries=entries+1
                if format~=2 then
                    db[key]={value1,value2}
                else
                    db[counter]={key,value1,value2}
                    counter=counter+1
                end
            end
        end
        if looping==false then
            function db:add(key,value1,value2)
                db[key]={value1,value2}
            end
            function db:getValues(key)
                return db[key]
            end
        end
        if entries==0 then
...
function csvfile:parseFile(filename,sep,format,looping)
  ...
  return csvfile:parse(filename,sep,format,looping)
end
...
function csvfile:readFile(filename,sep)
  ...
  return csvfile:parse(filename,sep,2,true)
end
...
function csvfile:write(infile,tbl,sep,format)
    ...
    for k,v in pairs(tbl) do
        if tostring(v):sub(1,8)~='function' then
            if format~=2 then fout:write(string.format("%s%s%s%s%s\n",k,sep,tbl[k][1],sep,tbl[k][2]))
            else fout:write(string.format("%s%s%s%s%s\n",v[1],sep,v[2],sep,v[3])) end
        end
    end
    fout:close()
end
...
function csvfile:writeFile(filename,tbl,sep,format)
  ...
  csvfile:write(filename,tbl,sep,format)
end
...
function csvfile:appendFile(filename,tbl,sep,format)
  ...
  csvfile:append(filename,tbl,sep,format)
end
